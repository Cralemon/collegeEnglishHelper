...
- **卡片式层叠翻卡设计**：左滑划走当前卡片，右滑找回上一张
- **正面**：题目 + 翻译输入框 + 提交按钮
- **反面**：三维评分 + 问题列表 + 翻译策略分析 + 整体学习建议
- **翻译模式切换**：单句 / 段落（设置中切换）
- **题集管理**：支持创建、切换题集，缓存作答记录
- **打乱功能**：随机打乱当前题集顺序
...
- 分数趋势图：展示历史分数变化
- **再练习**：允许将某一题目集合（如 50 分以下、某改进点相关）推入首页重新练习

### 4.3 个人/设置页

- 用户信息：昵称、学年段、预估词汇量
- 应用设置：
  - 主题风格切换（Claude / 微软 / 谷歌 / iOS）
  - 外观模式（浅色 / 深色 / 跟随系统）
  - 翻译模式（单句 / 段落）
  - 翻译方向（中译英 / 英译中）
  - 题目偏好（预设类型 + 自定义）
- **用户画像展示**：weakCategories、recentTrend、各维度掌握度
- **水平测试**：快速评估用户英语水平，生成初始画像（Phase 10 实现）
...
- [ ] Tauri Android 打包
  ...
  - [ ] 导航栏安全区适配
- [ ] Tauri Windows 打包
  - [ ] 最小窗口大小配置（`app.windows[].minWidth/minHeight`）
- [ ] 性能优化（图片、字体、动画）
- [ ] 水平测试功能（快速生成用户画像）
